...
test("shows generic study rating failures", async () => {
  ...
  render(
    <ReviewPage
      mode="study"
      session={studySession}
      onRate={vi.fn().mockRejectedValue(new Error("offline"))}
      onRefresh={vi.fn()}
    />,
  );
...
test("shows source controls in Practice All for cards linked to a PDF", () => {
  ...
  render(<ReviewPage mode="practice" cards={[sourcedCard]} />);

  expect(screen.getAllByRole("button", { name: "View source" })).toHaveLength(2);
});

test("keeps unavailable source controls visible in Practice All", () => {
  render(<ReviewPage mode="practice" cards={[card]} />);
